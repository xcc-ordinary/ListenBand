...
ListenBand already has an existing Community directory entry. Do not create a
second entry with **Plugins → New plugin**.

1. Sign in at https://community.obsidian.md.
2. Connect the `xcc-ordinary` GitHub account to the Community profile.
3. Claim the existing **ListenBand** entry under **Available to claim**. If it
   has already been claimed, open **Plugins → Your entries → ListenBand**.
4. Confirm that the linked repository is still
   `https://github.com/xcc-ordinary/ListenBand`. Recreating the repository
...
7. On the ListenBand entry, select **Check for new releases** and verify that
